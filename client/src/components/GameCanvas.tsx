  ...
  return (
    <main className={`game-shell ${crisisState !== "normal" ? `is-${crisisState}` : ""}`}>
      ...
      <div className="signal-hud">
        ...
        {snapshot.mode === "battle" && <button type="button" className="pause-button" onClick={() => controller?.togglePause()} aria-pressed={snapshot.paused} aria-label="一時停止メニューを開く">{snapshot.paused ? "▶ 再開" : "Ⅱ 停止"}</button>}
        ...
        <section className={`player-console technical-panel ${crisisState !== "normal" ? "is-crisis" : ""}`}>
          ...
          <div className="metric-row"><span>INTEGRITY</span><strong className={crisisState !== "normal" ? "is-crisis" : ""}>{String(snapshot.playerHp).padStart(3, "0")}</strong></div>
          <div className={`meter hp-meter ${crisisState !== "normal" ? `is-${crisisState}` : ""}`}><span style={meterStyle(hpRatio)} /></div>
          {crisisState !== "normal" && <div className={`crisis-readout is-${crisisState}`}><i /> {crisisState === "critical" ? "CRITICAL // EVASIVE ACTION" : "CAUTION // INTEGRITY LOW"}</div>}
          <div className="metric-row compact"><span>POSITION</span><strong>{snapshot.playerGrid.col + 1} · {snapshot.playerGrid.row + 1}</strong></div>
        ...
        <section className="enemy-console technical-panel">
          ...
          {snapshot.enemies.map((enemy) => (
            <div className="enemy-readout" key={enemy.id}>
              <div><span className={`enemy-state state-${enemy.state}`} /> <b>{enemy.name}</b></div>
              ...
              <div className="meter enemy-meter"><span style={meterStyle((enemy.hp / enemy.maxHp) * 100)} /></div>
            </div>
          ))}
        </section>
        ...
        <section className="run-console technical-panel">
          ...
          <div><span>SCORE</span><strong>{String(snapshot.score).padStart(5, "0")}</strong></div>
          <small>BEST {String(snapshot.highScore).padStart(5, "0")} / WAVE {String(snapshot.bestWave).padStart(2, "0")}</small>
        </section>
          ...
          <>
            <section className="queue-console technical-panel">
              ...
              {snapshot.queue.length > 0 ? (
                <><strong>{snapshot.queue[0].name}</strong><span>{snapshot.sync ? snapshot.queue[0].power * 2 : snapshot.queue[0].power} OUT</span></>
              ) : <strong className="empty-queue">送信済みカードなし</strong>}
            </section>
            <section className="gauge-console technical-panel">
              <div className="metric-row"><span>CUSTOM IN</span><strong>{snapshot.customRemaining.toFixed(1)}S</strong></div>
              <div className="meter gauge-meter"><span style={meterStyle(snapshot.gauge)} /></div>
              <button type="button" onClick={() => controller?.openCustom()}>ROUTE C</button>
            </section>
            <section className="skill-rail" aria-label="送信済みカード">
              {snapshot.queue.map((card, index) => (
                <div className={`queued-card ${index === 0 ? "next" : ""} ${card.tier === "mega" ? "mega" : ""}`} key={`${card.id}-${index}`}><em>{index + 1} / {card.tier === "mega" ? "メガ" : card.family}</em><span>{card.name}</span><b>{card.power}</b></div>
              ))}
            </section>
          </>
        )}
        ...
        <section className="custom-console" aria-label="カスタムコンソール">
          ...
          <div className="custom-heading">
            ...
            <div className="card-inspector" aria-live="polite">
              {snapshot.focusedCard !== null ? <><b>{snapshot.customHand[snapshot.focusedCard]?.name}</b><span>{snapshot.customHand[snapshot.focusedCard]?.description}</span><em>もう一度タップして選択</em></> : <span>カードを1回タップすると説明を表示します。</span>}
            </div>
          </div>
          <div className="card-deck">
            {snapshot.customHand.map((card, index) => {
              const selected = snapshot.selected.includes(index); const focused = snapshot.focusedCard === index; const selectionOrder = snapshot.selected.indexOf(index) + 1;
              return (
          ...
          <section className="custom-range-preview" aria-label="カード攻撃範囲プレビュー">
            <div className="range-preview-label"><span>RANGE MAP / {snapshot.customHand[snapshot.focusedCard ?? snapshot.selected[0] ?? 0]?.target ?? "front"}</span><b>{snapshot.customHand[snapshot.focusedCard ?? snapshot.selected[0] ?? 0]?.name ?? "カード選択"}</b></div>
            <div className={`range-board range-${snapshot.customHand[snapshot.focusedCard ?? snapshot.selected[0] ?? 0]?.target ?? "front"}`}>
              {Array.from({ length: 18 }, (_, index) => {
                const previewCard = snapshot.customHand[snapshot.focusedCard ?? snapshot.selected[0] ?? 0]; const col = Math.floor(index / 3); const row = index % 3; const tileKey = `${col}:${row}`; const target = previewTargets(previewCard).has(tileKey); const player = tileKey === "1:1";
                return <span className={`range-tile ${col <= 2 ? "ally" : "enemy"} ${target ? "target animated-target" : ""} ${player ? "player" : ""}`} style={target ? { animationDelay: `${previewDelay(previewCard, col, row)}ms` } : undefined} key={tileKey}>{player && <i>P</i>}</span>;
              })}
            </div>
            <small><b className="range-vector">{previewVector(snapshot.customHand[snapshot.focusedCard ?? snapshot.selected[0] ?? 0])}</b> 自陣中央の P から、説明中または選択中のカードが作用する対象マスを順に走査</small>
          </section>
          <div className="custom-footer">
            <p><span>{snapshot.selected.length}</span> / 05 CARDS ROUTED</p>
            <button type="button" className="engage-button" onClick={() => controller?.confirmCustom()}>ENGAGE <span>↗</span></button>
          </div>
        </section>
      )}
      ...
      {snapshot.mode === "battle" && (
        <div className="mobile-controls" aria-label="タッチ操作">
          <div className="dpad dpad-large">
            <button type="button" aria-label="上へ移動" onPointerDown={(event) => { event.preventDefault(); controller?.move(0, 1); }}>▲</button>
            <button type="button" aria-label="左へ移動" onPointerDown={(event) => { event.preventDefault(); controller?.move(-1, 0); }}>◀</button>
            <button type="button" aria-label="右へ移動" onPointerDown={(event) => { event.preventDefault(); controller?.move(1, 0); }}>▶</button>
            <button type="button" aria-label="下へ移動" onPointerDown={(event) => { event.preventDefault(); controller?.move(0, -1); }}>▼</button>
          </div>
          <div className="action-buttons">
            <button type="button" className="action-fire" onPointerDown={(event) => { event.preventDefault(); controller?.fire(); }} aria-label="通常攻撃">FIRE</button>
            <button type="button" className="action-charge" onPointerDown={(event) => { event.preventDefault(); controller?.startCharge(); }} onPointerUp={() => controller?.releaseCharge()} onPointerCancel={() => controller?.releaseCharge()} aria-label="チャージショット">CHG</button>
            <button type="button" className="action-skill" onPointerDown={(event) => { event.preventDefault(); controller?.useSkill(); }} aria-label="次のカードを使用">CARD</button>
          </div>
          <div className="charge-indicator"><span style={meterStyle(snapshot.charging * 100)} /></div>
        </div>
      )}
      ...
      {snapshot.mode === "battle" && snapshot.paused && (
        <section className="pause-console" aria-modal="true" role="dialog" aria-label="一時停止と設定">
          ...
          <div className="pause-settings">
            <button type="button" className="sound-toggle" onClick={toggleSound} aria-pressed={soundEnabled}>SFX {soundEnabled ? "ON" : "OFF"}</button>
            <label className="volume-control">音量 <input type="range" min="0" max="100" value={soundVolume} onChange={(event) => updateVolume(Number(event.target.value))} aria-label="効果音の音量" disabled={!soundEnabled} /></label>
            <button type="button" className="vibration-toggle" onClick={toggleVibration} aria-pressed={vibrationEnabled}>振動 {vibrationEnabled ? "ON" : "OFF"}</button>
          </div>
          <button type="button" className="engage-button" onClick={() => controller?.togglePause()}>RESUME <span>↗</span></button>
        </section>
      )}
      ...
      {snapshot.mode === "intermission" && (
        <section className="result-console wave-clear" aria-live="polite">
          ...
          <h2>ROUTE<br />THE NEXT WAVE</h2>
          <div className="wave-reward"><span>INTEGRITY RESTORED</span><strong>+32</strong><small>WAVE 0{snapshot.wave + 1} INBOUND</small></div>
          <div className="result-details"><span>RUN SCORE <b>{snapshot.score}</b></span><span>COUNTERS <b>{String(snapshot.counters).padStart(2, "0")}</b></span></div>
          <button type="button" className="engage-button" onClick={() => controller?.nextWave()}>NEXT WAVE <span>↗</span></button>
        </section>
      )}
      ...
      {snapshot.mode === "result" && (
        <section className="result-console" aria-live="polite">
          ...
          <div className="rank-dial"><span>RANK</span><strong>{snapshot.rank}</strong></div>
          <div className="result-details"><span>SCORE <b>{snapshot.score}</b></span><span>BEST <b>{snapshot.highScore}</b></span><span>WAVE <b>{String(snapshot.bestWave).padStart(2, "0")}</b></span></div>
          <button type="button" className="engage-button" onClick={() => controller?.restart()}>RELINK <span>↗</span></button>
        </section>
      )}
